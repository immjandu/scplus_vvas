...
#endif
#ifdef ENABLE_FACERECOG
#include "vvas_xfacerecog.hpp"
#endif
#ifdef ENABLE_FACEDETECTRECOG
...
vvas_xinitmodel (vvas_xkpriv * kpriv, int modelclass)
{
  ...
  kpriv->embeddings_npzpath = kpriv->modelpath + "/" + kpriv->modelname + "/embeddings.npz";
  if (fileexists(kpriv->embeddings_npzpath))
    LOG_MESSAGE (LOG_LEVEL_DEBUG, kpriv->log_level, "embeddings.npz found\n");
  ...
  switch (modelclass) {
    ...
    {
      model =
          new vvas_xfacefeature (kpriv, kpriv->elfname, kpriv->need_preprocess);
      break;
    }
#endif
#ifdef ENABLE_FACERECOG
    case VVAS_XCLASS_FACERECOG:
    {
      model =
          new vvas_xfacerecog (kpriv, kpriv->elfname, kpriv->need_preprocess);
      break;
    }
#endif
#ifdef ENABLE_FACEDETECTRECOG
    case VVAS_XCLASS_FACEDETECTRECOG:
    {
      model =
          new vvas_xfacedetectrecog (kpriv, kpriv->elfname, kpriv->need_preprocess);
      break;
    }
#endif
#ifdef ENABLE_REID
