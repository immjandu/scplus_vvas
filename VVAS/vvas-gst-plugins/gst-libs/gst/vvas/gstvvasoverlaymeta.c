    ...
    GstBuffer * buffer, GQuark type, gpointer data)
{
  ...
  // for(int n_rect=0; n_rect < smeta->num_rects; n_rect++) {
  //   printf("gst_vvas_overlay_meta_transform >> smeta->rects: %d x %d (%d x %d) \n",
  //   smeta->rects[n_rect].width, smeta->rects[n_rect].height, smeta->rects[n_rect].offset.x, smeta->rects[n_rect].offset.y);
  // }
  return ret;
}
